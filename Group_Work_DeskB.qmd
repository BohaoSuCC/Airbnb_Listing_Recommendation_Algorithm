...
```
+ label: Figure3a
#+ fig.cap: "Figure3a: Statistical distribution of Listings' profit-cost ratio"
...
```
+ label: Figure3b
#+ fig.cap: "Figure3b: spatial distribution of Listings' profit-cost ratio"

We multiple the price of each listing by its total nights in the last year, total nights was calculated by minimum nights, maximum nights and number of reviews. Though, technically this is an approximate number with normal distribution[@Figure3a], but it aligns with the data from the [\@Inside Airbnb](http://insideairbnb.com/london). Afterwards, we compare 'sum_income' with the average property value to get an integrated index to indicate this listing's 'cost-benefit ratio'. Then we standarlize the data and visualize it in the map[@Figure3b]. The blue area means these listings have the potential to make more profit and fullfill more contract, which should be highlighted when branding and promoting.
