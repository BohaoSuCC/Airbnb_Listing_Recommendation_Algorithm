...
We could use the LDA model to generalize and extract topics to get the most frequent keywords in those topics. After calculating iteratively the model, we determine the best topics' number for summarizing 'descriptions' column should be 16. ([***Figure1a***]{.underline})
...
# labels of the picture on the bottom
fig.text(0.5, 0.05, 'Figure1b: Topics and key words', ha='center', va='bottom', fontsize=30)
...
# 添加题注
fig.text(0.5, 0.05, 'Figure2a: Features clustering after UMAP', ha='center', va='bottom', fontsize=12)
...
# 添加题注
fig.text(0.5, 0.15, 'Figure2b: spatial distribution of Listings  similarities', ha='center', va='bottom', fontsize=16)
...
# 添加题注
fig.text(0.5, 0.07, 'Figure3a: Statistical distribution of Listings\' profit-cost ratio', ha='center', va='bottom', fontsize=12)
...
# 添加题注
fig.text(0.5, 0.2, 'Figure3b: spatial distribution of Listings profit-cost ratio', ha='center', va='bottom', fontsize=16)
...
We use several numeric columns to calculate the total income for every listing. Though, technically this is an approximate number with normal distribution [***Figure3a***]{.underline}, but it aligns with the data from the [Inside Airbnb](http://insideairbnb.com/london). Afterwards, we compare 'sum_income' with the average property in that wards to indicate this listing's 'cost-benefit ratio'. Then we standarlize the data and visualize it in the map [***Figure3b***]{.underline}. The blue area means these listings have the potential to make more profit and fullfill more contract, which should be highlighted and coordinated with \[***Figure2b***\] when branding and promoting.
